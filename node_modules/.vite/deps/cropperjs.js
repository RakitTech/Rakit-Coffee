//#region node_modules/@cropper/utils/dist/utils.esm.raw.js
...
function getComposedPathTarget(event) {
	if (typeof event.composedPath === "function") return event.composedPath().find(isElement) || event.target;
	return event.target;
}
...
function getRootDocument(element) {
	...
	switch (rootNode.nodeType) {
		case 1: return rootNode.ownerDocument;
		case 9: return rootNode;
		case 11: return rootNode;
	}
	return null;
}
/**
...
function toAngleInRadian(angle) {
	...
	if (value !== 0) {
		...
		switch (unit.toLowerCase()) {
			case "deg": return value / 360 * (Math.PI * 2);
			case "grad": return value / 400 * (Math.PI * 2);
			case "turn": return value * (Math.PI * 2);
		}
	}
	return value;
}
...
function getAdjustedSizes(data, type = SIZE_ADJUSTMENT_TYPE_CONTAIN) {
	...
	if (isValidWidth && isValidHeight) {
		...
		if (type === SIZE_ADJUSTMENT_TYPE_CONTAIN && adjustedWidth > width || type === SIZE_ADJUSTMENT_TYPE_COVER && adjustedWidth < width) height = width / aspectRatio;
...
//#endregion
//#region node_modules/@cropper/element/dist/element.esm.raw.js
...
var CropperElement = class extends HTMLElement {
	...
	constructor() {
		...
		if (name) tagNames.set(name, this.tagName.toLowerCase());
	}
	...
	attributeChangedCallback(name, oldValue, newValue) {
		...
		switch (name) {
			case "theme-color": {
				...
				if (styleSheet && styles) if (supportsAdoptedStyleSheets) styleSheet.replaceSync(styles);
				else styleSheet.textContent = styles;
				break;
			}
		}
	}
	$propertyChangedCallback(name, oldValue, newValue) {
		...
		switch (typeof newValue) {
			case "boolean":
				...
				break;
			case "number": if (isNaN(newValue)) newValue = "";
			else newValue = String(newValue);
			default: if (newValue) {
				if (this.getAttribute(name) !== newValue) this.setAttribute(name, newValue);
			} else this.removeAttribute(name);
		}
	}
	connectedCallback() {
		Object.getPrototypeOf(this).constructor.observedAttributes.forEach((attribute) => {
			...
			if (!isUndefined(value)) this.$propertyChangedCallback(property, void 0, value);
		...
		const shadow = this.shadowRoot || this.attachShadow({ mode: this.shadowRootMode || DEFAULT_SHADOW_ROOT_MODE });
		shadowRoots.set(this, shadow);
		styleSheets.set(this, this.$addStyles(this.$sharedStyle));
		if (this.$style) this.$addStyles(this.$style);
		if (this.$template) {
	...
	disconnectedCallback() {
		if (styleSheets.has(this)) styleSheets.delete(this);
		if (shadowRoots.has(this)) shadowRoots.delete(this);
	}
	$getTagNameOf(name) {
		var _a;
	...
	$setStyles(properties) {
		Object.keys(properties).forEach((property) => {
			...
			if (isNumber(value)) if (value !== 0 && REGEXP_SUFFIX.test(property)) value = `${value}px`;
	...
	*/
	static $define(name, options) {
		...
		if (!name) name = this.$name || this.name;
		name = toKebabCase(name);
		if (IS_BROWSER && WINDOW.customElements && !WINDOW.customElements.get(name)) customElements.define(name, this, options);
	}
};
CropperElement.$version = "2.1.1";
//#endregion
//#region node_modules/@cropper/element-canvas/dist/element-canvas.esm.raw.js
...
var CropperCanvas = class extends CropperElement {
	...
	$unbind() {
		...
		if (this.$onWheel) {
			off(this, EVENT_WHEEL, this.$onWheel, { capture: true });
			this.$onWheel = null;
		}
	}
	$handlePointerDown(event) {
		const { buttons, button, type } = event;
		if (this.disabled || (type === "pointerdown" && event.pointerType === "mouse" || type === "mousedown") && (isNumber(buttons) && buttons !== 1 || isNumber(button) && button !== 0 || event.ctrlKey)) return;
		const { $pointers } = this;
		let action = "";
		if (event.changedTouches) Array.from(event.changedTouches).forEach(({ identifier, pageX, pageY }) => {
		...
		if ($pointers.size > 1) action = ACTION_TRANSFORM;
		else if (isElement(event.target)) action = event.target.action || event.target.getAttribute("action") || "";
		if (this.$emit("actionstart", {
			action,
			relatedEvent: event
		}) === false) return;
	...
	$handlePointerMove(event) {
		...
		if (this.disabled || $action === "none" || $pointers.size === 0) return;
		if (this.$emit("actionmove", {
			action: $action,
			relatedEvent: event
		}) === false) return;
		event.preventDefault();
		if (event.changedTouches) Array.from(event.changedTouches).forEach(({ identifier, pageX, pageY }) => {
			...
			if (pointer) Object.assign(pointer, {
				endX: pageX,
				endY: pageY
			});
		});
		...
		if ($action === "transform") {
			...
			$pointers.forEach((pointer, pointerId) => {
				...
				pointers2.forEach((pointer2) => {
					...
					let a2 = 0;
					if (x1 === 0) {
						if (y1 < 0) a1 = Math.PI * 2;
						else if (y1 > 0) a1 = Math.PI;
					} else if (x1 > 0) a1 = Math.PI / 2 + Math.atan(y1 / x1);
					else if (x1 < 0) a1 = Math.PI * 1.5 + Math.atan(y1 / x1);
					if (x2 === 0) {
						if (y2 < 0) a2 = Math.PI * 2;
						else if (y2 > 0) a2 = Math.PI;
					} else if (x2 > 0) a2 = Math.PI / 2 + Math.atan(y2 / x2);
					else if (x2 < 0) a2 = Math.PI * 1.5 + Math.atan(y2 / x2);
					if (a2 > 0 || a1 > 0) {
					...
					x2 = Math.abs(x2);
					y2 = Math.abs(y2);
					if (x1 > 0 && y1 > 0) z1 = Math.sqrt(x1 * x1 + y1 * y1);
					else if (x1 > 0) z1 = x1;
					else if (y1 > 0) z1 = y1;
					if (x2 > 0 && y2 > 0) z2 = Math.sqrt(x2 * x2 + y2 * y2);
					else if (x2 > 0) z2 = x2;
					else if (y2 > 0) z2 = y2;
					if (z1 > 0 && z2 > 0) {
			...
			if (rotatable && scalable) {
				...
				detail.centerX = centerX;
				detail.centerY = centerY;
			} else detail.action = ACTION_NONE;
		} else {
		...
		if (detail.action !== "none") this.$emit(EVENT_ACTION, detail, { cancelable: false });
	}
	$handlePointerUp(event) {
		const { $action, $pointers } = this;
		if (this.disabled || $action === "none") return;
		if (this.$emit("actionend", {
			action: $action,
			relatedEvent: event
		}) === false) return;
		event.preventDefault();
		if (event.changedTouches) Array.from(event.changedTouches).forEach(({ identifier }) => {
	...
	$handleWheel(event) {
		...
		const scale = (event.deltaY > 0 ? -1 : 1) * this.scaleStep;
	...
	*/
	$setAction(action) {
		if (isString(action)) this.$action = action;
		return this;
	}
	/**
	...
	*/
	$toCanvas(options) {
		return new Promise((resolve, reject) => {
			if (!this.isConnected) {
				reject(/* @__PURE__ */ new Error("The current element is not connected to the DOM."));
				return;
			}
			...
			cropperImage.$ready().then((image) => {
				...
				if (context) {
					...
					if (isPlainObject(options) && isFunction(options.beforeDraw)) options.beforeDraw.call(this, context, canvas);
					context.save();
...
//#endregion
//#region node_modules/@cropper/element-image/dist/element-image.esm.raw.js
...
var canvasCache$3 = /* @__PURE__ */ new WeakMap();
...
var CropperImage = class extends CropperElement {
	...
	attributeChangedCallback(name, oldValue, newValue) {
		...
		if (NATIVE_ATTRIBUTES.includes(name)) this.$image.setAttribute(name, newValue);
	}
	$propertyChangedCallback(name, oldValue, newValue) {
		if (Object.is(newValue, oldValue)) return;
		super.$propertyChangedCallback(name, oldValue, newValue);
	...
	$handleLoad() {
		...
		if (this.$canvas) this.$center(this.initialCenterSize);
		this.$isReady = true;
	}
	$handleAction(event) {
		...
		if (detail) {
			...
			let { action } = detail;
			if (action === "transform" && (!this.rotatable || !this.scalable)) if (this.rotatable) action = ACTION_ROTATE;
			else if (this.scalable) action = ACTION_SCALE;
			else action = ACTION_NONE;
			switch (action) {
				case ACTION_MOVE:
					if (this.translatable) {
						let $selection = null;
						if (relatedEvent) $selection = relatedEvent.target.closest(this.$getTagNameOf(CROPPER_SELECTION));
						if (!$selection) $selection = $canvas.querySelector(this.$getTagNameOf(CROPPER_SELECTION));
						if ($selection && $selection.multiple && !$selection.active) $selection = $canvas.querySelector(`${this.$getTagNameOf(CROPPER_SELECTION)}[active]`);
						if (!$selection || $selection.hidden || !$selection.movable || $selection.dynamic || !(this.$actionStartTarget && $selection.contains(this.$actionStartTarget))) this.$move(detail.endX - detail.startX, detail.endY - detail.startY);
					}
					break;
				case ACTION_ROTATE:
					if (this.rotatable) if (relatedEvent) {
						const { x, y } = this.getBoundingClientRect();
						this.$rotate(detail.rotate, relatedEvent.clientX - x, relatedEvent.clientY - y);
					} else this.$rotate(detail.rotate);
					break;
				case ACTION_SCALE:
					if (this.scalable) if (relatedEvent) {
 ...
 /**
	...
	*/
	$ready(callback) {
		...
		const promise = new Promise((resolve, reject) => {
			const error = /* @__PURE__ */ new Error("Failed to load the image source");
			if ($image.complete) if ($image.naturalWidth > 0 && $image.naturalHeight > 0) resolve($image);
			else reject(error);
			else {
				const onLoad = () => {
		...
		if (isFunction(callback)) promise.then((image) => {
			callback(image);
			return image;
		});
		return promise;
	...
	$rotate(angle, x, y) {
		if (this.rotatable) {
			...
			if (isNumber(x) && isNumber(y)) {
				...
				*/
				this.$transform(scaleX, skewY, skewX, scaleY, translateX * (1 - scaleX) - translateY * skewX, translateY * (1 - scaleY) - translateX * skewY);
			} else this.$transform(scaleX, skewY, skewX, scaleY, 0, 0);
		}
		return this;
	}
	/**
	...
	*/
	$zoom(scale, x, y) {
		if (!this.scalable || scale === 0) return this;
		if (scale < 0) scale = 1 / (1 - scale);
		else scale += 1;
		if (isNumber(x) && isNumber(y)) {
			const [a, b, c, d] = this.$matrix;
	...
	*/
	$scale(x, y = x) {
		if (this.scalable) this.$transform(x, 0, 0, y, 0, 0);
		return this;
	}
	/**
	* Skews the image.
	...
	*/
	$translate(x, y = x) {
		if (this.translatable && isNumber(x) && isNumber(y)) this.$transform(1, 0, 0, 1, x, y);
		return this;
	}
	/**
	...
	*/
	$transform(a, b, c, d, e, f) {
		if (isNumber(a) && isNumber(b) && isNumber(c) && isNumber(d) && isNumber(e) && isNumber(f)) return this.$setTransform(multiplyMatrices(this.$matrix, [
			a,
			b,
			c,
			d,
	...
	*/
	$setTransform(a, b, c, d, e, f) {
		if (this.rotatable || this.scalable || this.skewable || this.translatable) {
			if (Array.isArray(a)) [a, b, c, d, e, f] = a;
...
//#endregion
//#region node_modules/@cropper/element-shade/dist/element-shade.esm.raw.js
...
var canvasCache$2 = /* @__PURE__ */ new WeakMap();
var CropperShade = class extends CropperElement {
	...
	connectedCallback() {
		...
		if ($canvas) {
			...
			if ($selection) {
				...
				this.$onCanvasActionStart = (event) => {
					if ($selection.hidden && event.detail.action === "select") this.hidden = false;
				};
				this.$onCanvasActionEnd = (event) => {
					if ($selection.hidden && event.detail.action === "select") this.hidden = true;
				};
				this.$onSelectionChange = (event) => {
					const { x, y, width, height } = event.defaultPrevented ? $selection : event.detail;
					this.$change(x, y, width, height);
					if ($selection.hidden || x === 0 && y === 0 && width === 0 && height === 0) this.hidden = true;
				};
	...
	*/
	$change(x, y, width = this.width, height = this.height) {
		if (!isNumber(x) || !isNumber(y) || !isNumber(width) || !isNumber(height) || x === this.x && y === this.y && width === this.width && height === this.height) return this;
...
//#endregion
//#region node_modules/@cropper/element-handle/dist/element-handle.esm.raw.js
...
//#endregion
//#region node_modules/@cropper/element-selection/dist/element-selection.esm.raw.js
...
var canvasCache$1 = /* @__PURE__ */ new WeakMap();
var CropperSelection = class extends CropperElement {
	...
	$propertyChangedCallback(name, oldValue, newValue) {
		...
		switch (name) {
			...
			case "width":
			case "height":
				if (!this.$changing) this.$nextTick(() => {
			...
			case "initialCoverage":
				this.$nextTick(() => {
					if (isPositiveNumber(newValue) && newValue <= 1) this.$initSelection(true, true);
				});
				break;
			case "keyboard":
				this.$nextTick(() => {
	...
	$getSelections() {
		let selections = [];
		if (this.parentElement) selections = Array.from(this.parentElement.querySelectorAll(this.$getTagNameOf(CROPPER_SELECTION)));
		return selections;
	}
	$initSelection(center = false, resize = false) {
		...
		if (isPositiveNumber(initialCoverage) && parentElement) {
			...
			if (isPositiveNumber(aspectRatio)) ({width, height} = getAdjustedSizes({
				aspectRatio,
				width,
				height
			}));
	...
	$createSelection() {
		...
		if (this.hasAttribute("id")) newSelection.removeAttribute("id");
		newSelection.initialCoverage = NaN;
		this.active = false;
		if (this.parentElement) this.parentElement.insertBefore(newSelection, this.nextSibling);
		return newSelection;
	}
	...
	$handleAction(event) {
		...
		if (!currentTarget || !detail) return;
		const { relatedEvent } = detail;
		let { action } = detail;
		...
		if (!action || this.hidden && action !== "select" || this.multiple && !this.active && action !== "scale") return;
		const { width, height } = this;
		let moveX = detail.endX - detail.startX;
		let moveY = detail.endY - detail.startY;
		let { aspectRatio } = this;
		if (!isPositiveNumber(aspectRatio) && relatedEvent.shiftKey) aspectRatio = isPositiveNumber(width) && isPositiveNumber(height) ? width / height : 1;
		switch (action) {
			case ACTION_SELECT:
				if (moveX !== 0 || moveY !== 0) {
					...
					if (moveX < 0) {
						if (moveY < 0) action = ACTION_RESIZE_NORTHWEST;
						else if (moveY > 0) action = ACTION_RESIZE_SOUTHWEST;
					} else if (moveX > 0) {
						if (moveY < 0) action = ACTION_RESIZE_NORTHEAST;
						else if (moveY > 0) action = ACTION_RESIZE_SOUTHEAST;
					}
					if ($canvas) $canvas.$action = action;
				}
				break;
			case ACTION_MOVE:
				if (this.movable && (this.dynamic || this.$actionStartTarget && this.contains(this.$actionStartTarget))) this.$move(moveX, moveY);
				break;
			case ACTION_SCALE:
				...
				break;
			default: this.$resize(action, moveX, moveY, aspectRatio);
		}
	}
	$handleActionEnd() {
		this.$action = "";
	...
	$handleKeyDown(event) {
		if (this.hidden || !this.keyboard || this.multiple && !this.active || event.defaultPrevented) return;
		const { activeElement } = document;
		if (activeElement && (["INPUT", "TEXTAREA"].includes(activeElement.tagName) || ["true", "plaintext-only"].includes(activeElement.contentEditable))) return;
		switch (event.key) {
			case "Backspace":
				if (event.metaKey) {
					event.preventDefault();
	...
	$resize(action, offsetX = 0, offsetY = 0, aspectRatio = this.aspectRatio) {
		...
		switch (action) {
			...
			case ACTION_RESIZE_NORTHEAST:
				if (hasValidAspectRatio) offsetY = -offsetX / aspectRatio;
				y += offsetY;
				height -= offsetY;
				width += offsetX;
				if (width < 0 && height < 0) {
		...
		if ($canvas) $canvas.$setAction(action);
	...
	*/
	$zoom(scale, x, y) {
		if (!this.zoomable || scale === 0) return this;
		if (scale < 0) scale = 1 / (1 - scale);
		else scale += 1;
		const { width, height } = this;
	...
	*/
	$change(x, y, width = this.width, height = this.height, aspectRatio = this.aspectRatio, _force = false) {
		if (this.$changing || !isNumber(x) || !isNumber(y) || !isNumber(width) || !isNumber(height) || width < 0 || height < 0) return this;
		if (isPositiveNumber(aspectRatio)) ({width, height} = getAdjustedSizes({
			aspectRatio,
			width,
			height
		...
		if (x === this.x && y === this.y && width === this.width && height === this.height && Object.is(aspectRatio, this.aspectRatio) && !_force) return this;
		if (this.hidden) this.hidden = false;
		if (this.$emit("change", {
			x,
			y,
...
//#endregion
//#region node_modules/@cropper/element-grid/dist/element-grid.esm.raw.js
...
var CropperGrid = class extends CropperElement {
	...
	$propertyChangedCallback(name, oldValue, newValue) {
		if (Object.is(newValue, oldValue)) return;
		super.$propertyChangedCallback(name, oldValue, newValue);
		if (name === "rows" || name === "columns") this.$nextTick(() => {
			this.$render();
		});
	}
	connectedCallback() {
...
//#endregion
//#region node_modules/@cropper/element-crosshair/dist/element-crosshair.esm.raw.js
...
//#endregion
//#region node_modules/@cropper/element-viewer/dist/element-viewer.esm.raw.js
...
var CropperViewer = class extends CropperElement {
	...
	connectedCallback() {
		...
		super.connectedCallback();
		let $selection = null;
		if (this.selection) $selection = (_b = (_a = getRootDocument(this)) === null || _a === void 0 ? void 0 : _a.querySelector(this.selection)) !== null && _b !== void 0 ? _b : null;
		else $selection = this.closest(this.$getTagNameOf(CROPPER_SELECTION));
	...
	$render(selection, matrix) {
		...
		if (!selection && !$selection.hidden) selection = $selection;
		if (!selection || selection.x === 0 && selection.y === 0 && selection.width === 0 && selection.height === 0) selection = {
			x: 0,
			y: 0,
		...
		switch (this.resize) {
			...
			case RESIZE_VERTICAL:
				...
				break;
			case RESIZE_NONE:
			default: if (clientWidth > 0) scale = width > 0 ? clientWidth / width : 0;
			else if (clientHeight > 0) scale = height > 0 ? clientHeight / height : 0;
		}
		this.$scale = scale;
		this.$setStyles(styles);
		if (this.$sourceImage) setTimeout(() => {
...
//#endregion
//#region node_modules/cropperjs/dist/cropper.esm.raw.js
...
var DEFAULT_OPTIONS = { template: DEFAULT_TEMPLATE };
...
var Cropper = class {
	constructor(element, options) {
		...
		if (!isElement(element) || !REGEXP_ALLOWED_ELEMENTS.test(element.localName)) throw new Error("The first argument is required and must be an <img> or <canvas> element.");
		this.element = element;
		options = Object.assign(Object.assign({}, DEFAULT_OPTIONS), options);
		this.options = options;
		let { container } = options;
		if (container) {
			if (isString(container)) container = (_a = getRootDocument(element)) === null || _a === void 0 ? void 0 : _a.querySelector(container);
			if (!isElement(container)) throw new Error("The `container` option must be an element or a valid selector.");
		}
		if (!isElement(container)) if (element.parentElement) container = element.parentElement;
		else container = element.ownerDocument.body;
		this.container = container;
		const tagName = element.localName;
		let src = "";
		if (tagName === "img") ({src} = element);
		else if (tagName === "canvas" && window.HTMLCanvasElement) src = element.toDataURL();
		...
		if (template && isString(template)) {
			...
			Array.from(documentFragment.querySelectorAll(CROPPER_IMAGE)).forEach((image) => {
				...
				if (tagName === "img") [
					...
					if (element.hasAttribute(attribute)) image.setAttribute(attribute, element.getAttribute(attribute) || "");
				});
			});
			if (element.parentElement) {
				element.style.display = "none";
				container.insertBefore(documentFragment, element.nextSibling);
			} else container.appendChild(documentFragment);
		}
	}
	getCropperCanvas() {
	...
	destroy() {
		...
		if (cropperCanvas) (_a = cropperCanvas.parentElement) === null || _a === void 0 || _a.removeChild(cropperCanvas);
