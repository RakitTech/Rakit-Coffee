import{n as e,r as t,t as n}from"./store-L3pErRld.js";/* empty css                      */t((()=>{e();var t=[],r=[];document.addEventListener(`DOMContentLoaded`,()=>{i(),o(),s(),l(),d(),p(),window.addEventListener(`scroll`,()=>{if(a!==void 0&&a)return;let e=Array.from(document.querySelectorAll(`.category-section`)),t=document.querySelectorAll(`.category-tab`),n=document.getElementById(`category-indicator`);if(e.length===0||t.length===0)return;let r=null;for(let t=e.length-1;t>=0;t--)if(e[t].getBoundingClientRect().top<=350){r=e[t];break}if(r||=e[0],r){let e=r.querySelector(`.category-heading`);if(!e)return;let i=e.textContent.trim();t.forEach(e=>{if(e.textContent.trim()===i&&!e.classList.contains(`active`)&&(t.forEach(e=>e.classList.remove(`active`)),e.classList.add(`active`),n)){n.style.width=e.offsetWidth+`px`,n.style.left=e.offsetLeft+`px`;let t=e.closest(`.category-list`);if(t){let n=e.offsetLeft-t.clientWidth/2+e.clientWidth/2;t.scrollTo({left:n,behavior:`smooth`})}}})}},{passive:!0}),n.subscribe(()=>{i(),r.length>0&&p()})});function i(){let e=document.getElementById(`menu-container`),r=n.getMenu();e.innerHTML=``,[...new Set(r.map(e=>e.category))].forEach(n=>{let i=r.filter(e=>e.category===n);if(i.length>0){let r=document.createElement(`div`);r.className=`category-section`,r.id=`cat-`+n.replace(/\s+/g,`-`);let a=document.createElement(`h2`);a.className=`category-heading`,a.textContent=n,r.appendChild(a);let o=document.createElement(`div`);o.className=`menu-grid`,i.forEach(e=>{let n=document.createElement(`article`);n.className=`menu-card ${e.available?``:`disabled`}`;let r=t.filter(t=>t.id===e.id).reduce((e,t)=>e+t.qty,0),i=``;e.available&&(i=r>0?`
    ...
    `,o.appendChild(n)}),r.appendChild(o),e.appendChild(r)}})}var a=!1;function o(){let e=document.querySelectorAll(`.nav-item`),t=document.querySelectorAll(`.view-section`);e.forEach(n=>{n.addEventListener(`click`,r=>{r.preventDefault();let i=n.getAttribute(`href`).substring(1);e.forEach(e=>e.classList.remove(`active`)),n.classList.add(`active`),t.forEach(e=>{e.id===i?e.classList.add(`active`):e.classList.remove(`active`)}),d(),i===`view-cart`&&f()})}),document.getElementById(`floating-cart-bar`).addEventListener(`click`,()=>{document.querySelector(`.nav-item[href="#view-cart"]`).click()})}function s(){let e=document.querySelectorAll(`.category-tab`),t=document.getElementById(`category-indicator`);function n(e){!t||!e||(t.style.width=e.offsetWidth+`px`,t.style.left=e.offsetLeft+`px`)}let r=document.querySelector(`.category-tab.active`);r&&setTimeout(()=>n(r),100),e.forEach(t=>{t.addEventListener(`click`,()=>{a=!0,setTimeout(()=>{a=!1},800);let r=t.textContent.trim();if(r===`SEMUA`)window.scrollTo({top:0,behavior:`smooth`});else{let e=`cat-`+r.replace(/\s+/g,`-`),t=document.getElementById(e);t&&t.scrollIntoView({behavior:`smooth`})}e.forEach(e=>e.classList.remove(`active`)),t.classList.add(`active`),n(t);let i=t.closest(`.category-list`);if(i){let e=t.offsetLeft-i.clientWidth/2+t.clientWidth/2;i.scrollTo({left:e,behavior:`smooth`})}})})}window.recalculateModalPrice=function(){let e=document.getElementById(`modal-price`),t=parseInt(e.dataset.basePrice)||0,n=parseInt(document.getElementById(`modal-quantity-val`).value)||1,r=0;document.querySelectorAll(`.modifier-input:checked`).forEach(e=>{r+=parseInt(e.dataset.price)||0}),e.textContent=`Rp ${((t+r)*n).toLocaleString(`id-ID`)}`};function c(e,t=null){let n=document.getElementById(`dynamic-modifiers-container`);if(!n||(n.innerHTML=``,!e.modifierGroups||e.modifierGroups.length===0))return;let r=t&&t.selectedModifiers||[];e.modifierGroups.forEach((e,i)=>{let a=e.type===`single`,o=a?`radio`:`checkbox`,s=`mod_${i}`,c=r.find(t=>t.groupName===e.name),l=c?c.selected.map(e=>e.name):[],u=e.options.map((n,r)=>{let i=l.includes(n.name)||a&&r===0&&!t,c=n.priceAdd>0?` (+Rp ${n.priceAdd.toLocaleString(`id-ID`)})`:``;return`
    ...
    `;n.insertAdjacentHTML(`beforeend`,d)})}window.openCustomizationModal=function(e){let t=n.getMenu().find(t=>t.id===e);t&&(document.getElementById(`modal-item-id`).value=t.id,document.getElementById(`modal-cart-index`).value=``,document.getElementById(`modal-title`).textContent=t.name,document.getElementById(`modal-desc`).textContent=t.desc,document.getElementById(`modal-img`).src=t.image,document.getElementById(`modal-price`).dataset.basePrice=t.price,c(t),document.getElementById(`customization-form`).reset(),document.getElementById(`modal-quantity-val`).value=1,document.getElementById(`modal-quantity-display`).textContent=`1`,recalculateModalPrice(),document.getElementById(`customization-modal`).classList.add(`active`),document.getElementById(`modal-overlay`).classList.add(`active`))},window.openEditModal=function(e){let r=t[e];if(!r)return;let i=n.getMenu().find(e=>e.id===r.id);if(!i)return;document.getElementById(`modal-item-id`).value=r.id,document.getElementById(`modal-cart-index`).value=e,document.getElementById(`modal-title`).textContent=r.name,document.getElementById(`modal-desc`).textContent=i.desc,document.getElementById(`modal-img`).src=r.image,document.getElementById(`modal-price`).dataset.basePrice=r.price,document.getElementById(`customization-form`).reset(),c(i,r);let a=document.querySelector(`textarea[name="note"]`);a&&r.note&&(a.value=r.note),document.getElementById(`modal-quantity-val`).value=r.qty,document.getElementById(`modal-quantity-display`).textContent=r.qty,recalculateModalPrice(),document.getElementById(`customization-modal`).classList.add(`active`),document.getElementById(`modal-overlay`).classList.add(`active`)},window.quickDecrement=function(e){let n=-1;for(let r=t.length-1;r>=0;r--)if(t[r].id===e){n=r;break}n!==-1&&(t[n].qty--,t[n].qty<=0&&t.splice(n,1),d(),f(),i())},window.updateModalQuantity=function(e){let t=document.getElementById(`modal-quantity-val`),n=document.getElementById(`modal-quantity-display`);document.getElementById(`modal-price`);let r=(parseInt(t.value)||1)+e;r<1&&(r=1),t.value=r,n.textContent=r,recalculateModalPrice()};function l(){let e=document.getElementById(`customization-modal`),r=document.getElementById(`modal-overlay`),a=()=>{e.classList.remove(`active`),r.classList.remove(`active`)};document.getElementById(`close-modal`).addEventListener(`click`,a),r.addEventListener(`click`,a),document.getElementById(`customization-form`).addEventListener(`submit`,e=>{e.preventDefault();let r=document.getElementById(`modal-item-id`).value,o=document.getElementById(`modal-cart-index`).value,s=n.getMenu().find(e=>e.id===r),c=new FormData(e.target),l=c.get(`note`),p=parseInt(c.get(`quantity`))||1,m=[],h=0;s.modifierGroups&&s.modifierGroups.forEach((e,t)=>{let n=document.querySelectorAll(`input[name="mod_${t}"]:checked`);if(n.length>0){let t=Array.from(n).map(e=>{let t=parseInt(e.dataset.price)||0;return h+=t,{name:e.value,priceAdd:t}});m.push({groupName:e.name,selected:t})}});let g={...s,note:l,qty:p,selectedModifiers:m,modifierTotal:h};if(o!==``){let e=parseInt(o);g.cartId=t[e].cartId,t[e]=g,d(),f(),i()}else g.cartId=Date.now().toString(),u(g);a()})}function u(e){t.push(e),d(),i()}function d(){let e=document.getElementById(`floating-cart-bar`),n=document.getElementById(`view-cart`).classList.contains(`active`);if(t.length>0&&!n){let n=t.reduce((e,t)=>e+(t.price+(t.modifierTotal||0))*t.qty,0);document.getElementById(`cart-count`).textContent=`${t.length} ITEM`,document.getElementById(`cart-total`).textContent=`Rp ${n.toLocaleString(`id-ID`)}`,e.style.display=`flex`}else e.style.display=`none`}function f(){let e=document.getElementById(`cart-items-container`),n=document.getElementById(`btn-checkout`);if(t.length===0){e.innerHTML=`<p class="text-center" style="padding: 32px 0;">Keranjang kosong.</p>`,n.disabled=!0,document.getElementById(`cart-page-total`).textContent=`Rp 0`;return}n.disabled=!1;let r=0;e.innerHTML=``,t.forEach((t,n)=>{let i=t.price+(t.modifierTotal||0);r+=i*t.qty;let a=``;t.sugar&&t.sugar!==`Normal`&&(a+=`<p class="menu-desc" style="margin-bottom: 2px;">Gula: ${t.sugar}</p>`),t.selectedModifiers&&t.selectedModifiers.length>0&&t.selectedModifiers.forEach(e=>{let t=e.selected.map(e=>e.name).join(`, `);a+=`<p class="menu-desc" style="margin-bottom: 2px;">${e.groupName}: ${t}</p>`});let o=document.createElement(`div`);o.className=`cart-item`,o.innerHTML=`
    ...
    `,e.appendChild(o)}),document.getElementById(`cart-page-total`).textContent=`Rp ${r.toLocaleString(`id-ID`)}`}window.removeFromCart=function(e){t.splice(e,1),d(),f(),i()},window.processCheckout=function(){let e=document.getElementById(`customer-name`).value;if(!e){alert(`Silakan masukkan nama Anda.`);return}if(t.length===0)return;let a=t.reduce((e,t)=>e+(t.price+(t.modifierTotal||0))*t.qty,0),o=n.addOrder({customerName:e,items:[...t],total:a,table:`12`});r.push(o.id),t=[],d(),i(),alert(`Pembayaran Berhasil! Pesanan sedang diproses.`),document.querySelector(`.nav-item[href="#view-tracker"]`).click(),p()};function p(){let e=document.getElementById(`tracker-empty-state`),t=document.getElementById(`tracker-active-state`);if(r.length===0){e&&(e.style.display=`flex`),t&&(t.style.display=`none`);return}e&&(e.style.display=`none`),t&&(t.style.display=`block`);let i=n.getOrders(),a=r.map(e=>i.find(t=>t.id===e)).filter(Boolean).reverse();t.innerHTML=``;let o={Diterima:1,Dimasak:2,Siap:3};a.forEach(e=>{let n=o[e.status]||1,r=document.createElement(`div`);r.style.marginBottom=`32px`,r.style.padding=`24px`,r.style.backgroundColor=`var(--color-surface-lowest)`,r.style.borderRadius=`var(--radius-lg)`,r.style.boxShadow=`var(--shadow-sm)`;let i=e.items.map(e=>{let t=``;e.sugar&&e.sugar!==`Normal`&&(t+=`<div style="color: var(--color-text-muted); font-size: 12px; margin-top: 2px;">Gula: ${e.sugar}</div>`),e.selectedModifiers&&e.selectedModifiers.length>0&&e.selectedModifiers.forEach(e=>{let n=e.selected.map(e=>e.name).join(`, `);t+=`<div style="color: var(--color-text-muted); font-size: 12px; margin-top: 2px;">${e.groupName}: ${n}</div>`});let n=e.price+(e.modifierTotal||0);return`
