import{n as e,r as t,t as n}from"./store-C-mszT6b.js";/* empty css                      */t((()=>{e();var t=[],r=[];function i(e=`Memproses...`){let t=document.getElementById(`global-loader`);t||(t=document.createElement(`div`),t.id=`global-loader`,t.innerHTML=`
    ...
    `,o.appendChild(n)}),i.appendChild(o),e.appendChild(i)}})}var s=!1;function c(){let e=document.querySelectorAll(`.nav-item`),t=document.querySelectorAll(`.view-section`);e.forEach(n=>{n.addEventListener(`click`,r=>{r.preventDefault();let i=n.getAttribute(`href`).substring(1);e.forEach(e=>e.classList.remove(`active`)),n.classList.add(`active`),t.forEach(e=>{e.id===i?e.classList.add(`active`):e.classList.remove(`active`)}),p(),i===`view-cart`&&m()})}),document.getElementById(`floating-cart-bar`).addEventListener(`click`,()=>{document.querySelector(`.nav-item[href="#view-cart"]`).click()})}function l(){let e=document.querySelectorAll(`.category-tab`),t=document.getElementById(`category-indicator`);function n(e){!t||!e||(t.style.width=e.offsetWidth+`px`,t.style.left=e.offsetLeft+`px`)}let r=document.querySelector(`.category-tab.active`);r&&setTimeout(()=>n(r),100),e.forEach(t=>{t.addEventListener(`click`,()=>{s=!0,setTimeout(()=>{s=!1},800);let r=t.textContent.trim();if(r===`SEMUA`)window.scrollTo({top:0,behavior:`smooth`});else{let e=`cat-`+r.replace(/\s+/g,`-`),t=document.getElementById(e);t&&t.scrollIntoView({behavior:`smooth`})}e.forEach(e=>e.classList.remove(`active`)),t.classList.add(`active`),n(t);let i=t.closest(`.category-list`);if(i){let e=t.offsetLeft-i.clientWidth/2+t.clientWidth/2;i.scrollTo({left:e,behavior:`smooth`})}})})}window.recalculateModalPrice=function(){let e=document.getElementById(`modal-price`),t=parseInt(e.dataset.basePrice)||0,n=parseInt(document.getElementById(`modal-quantity-val`).value)||1,r=0;document.querySelectorAll(`.modifier-input:checked`).forEach(e=>{r+=parseInt(e.dataset.price)||0}),e.textContent=`Rp ${((t+r)*n).toLocaleString(`id-ID`)}`};function u(e,t=null){let n=document.getElementById(`dynamic-modifiers-container`);if(!n||(n.innerHTML=``,!e.modifierGroups||e.modifierGroups.length===0))return;let r=t&&t.selectedModifiers||[];e.modifierGroups.forEach((e,i)=>{let a=e.type===`single`,o=a?`radio`:`checkbox`,s=`mod_${i}`,c=r.find(t=>t.groupName===e.name),l=c?c.selected.map(e=>e.name):[],u=e.options.map((n,r)=>{let i=l.includes(n.name)||a&&r===0&&!t,c=n.priceAdd>0?` (+Rp ${n.priceAdd.toLocaleString(`id-ID`)})`:``;return`
    ...
    `;n.insertAdjacentHTML(`beforeend`,d)})}window.openCustomizationModal=async function(e){i(`Memuat menu...`);let t=await n.getMenu();a();let r=t.find(t=>t.id===e);r&&(document.getElementById(`modal-item-id`).value=r.id,document.getElementById(`modal-cart-index`).value=``,document.getElementById(`modal-title`).textContent=r.name,document.getElementById(`modal-desc`).textContent=r.desc,document.getElementById(`modal-img`).src=r.image,document.getElementById(`modal-price`).dataset.basePrice=r.price,u(r),document.getElementById(`customization-form`).reset(),document.getElementById(`modal-quantity-val`).value=1,document.getElementById(`modal-quantity-display`).textContent=`1`,recalculateModalPrice(),document.getElementById(`customization-modal`).classList.add(`active`),document.getElementById(`modal-overlay`).classList.add(`active`))},window.openEditModal=async function(e){let r=t[e];if(!r)return;i(`Memuat...`);let o=await n.getMenu();a();let s=o.find(e=>e.id===r.id);if(!s)return;document.getElementById(`modal-item-id`).value=r.id,document.getElementById(`modal-cart-index`).value=e,document.getElementById(`modal-title`).textContent=r.name,document.getElementById(`modal-desc`).textContent=s.desc,document.getElementById(`modal-img`).src=r.image,document.getElementById(`modal-price`).dataset.basePrice=r.price,document.getElementById(`customization-form`).reset(),u(s,r);let c=document.querySelector(`textarea[name="note"]`);c&&r.note&&(c.value=r.note),document.getElementById(`modal-quantity-val`).value=r.qty,document.getElementById(`modal-quantity-display`).textContent=r.qty,recalculateModalPrice(),document.getElementById(`customization-modal`).classList.add(`active`),document.getElementById(`modal-overlay`).classList.add(`active`)},window.quickDecrement=function(e){let n=-1;for(let r=t.length-1;r>=0;r--)if(t[r].id===e){n=r;break}n!==-1&&(t[n].qty--,t[n].qty<=0&&t.splice(n,1),p(),m(),o())},window.updateModalQuantity=function(e){let t=document.getElementById(`modal-quantity-val`),n=document.getElementById(`modal-quantity-display`);document.getElementById(`modal-price`);let r=(parseInt(t.value)||1)+e;r<1&&(r=1),t.value=r,n.textContent=r,recalculateModalPrice()};function d(){let e=document.getElementById(`customization-modal`),r=document.getElementById(`modal-overlay`),s=()=>{e.classList.remove(`active`),r.classList.remove(`active`)};document.getElementById(`close-modal`).addEventListener(`click`,s),r.addEventListener(`click`,s),document.getElementById(`customization-form`).addEventListener(`submit`,async e=>{e.preventDefault(),i(`Menyiapkan...`);let r=await n.getMenu();a();let c=document.getElementById(`modal-item-id`).value,l=document.getElementById(`modal-cart-index`).value,u=r.find(e=>e.id===c),d=new FormData(e.target),h=d.get(`note`),g=parseInt(d.get(`quantity`))||1,_=[],v=0;u.modifierGroups&&u.modifierGroups.forEach((e,t)=>{let n=document.querySelectorAll(`input[name="mod_${t}"]:checked`);if(n.length>0){let t=Array.from(n).map(e=>{let t=parseInt(e.dataset.price)||0;return v+=t,{name:e.value,priceAdd:t}});_.push({groupName:e.name,selected:t})}});let y={...u,note:h,qty:g,selectedModifiers:_,modifierTotal:v};if(l!==``){let e=parseInt(l);y.cartId=t[e].cartId,t[e]=y,p(),m(),o()}else y.cartId=Date.now().toString(),f(y);s()})}function f(e){t.push(e),p(),o()}function p(){let e=document.getElementById(`floating-cart-bar`),n=document.getElementById(`view-cart`).classList.contains(`active`);if(t.length>0&&!n){let n=t.reduce((e,t)=>e+(t.price+(t.modifierTotal||0))*t.qty,0);document.getElementById(`cart-count`).textContent=`${t.length} ITEM`,document.getElementById(`cart-total`).textContent=`Rp ${n.toLocaleString(`id-ID`)}`,e.style.display=`flex`}else e.style.display=`none`}function m(){let e=document.getElementById(`cart-items-container`),n=document.getElementById(`btn-checkout`);if(t.length===0){e.innerHTML=`<p class="text-center" style="padding: 32px 0;">Keranjang kosong.</p>`,n.disabled=!0,document.getElementById(`cart-page-total`).textContent=`Rp 0`;return}n.disabled=!1;let r=0;e.innerHTML=``,t.forEach((t,n)=>{let i=t.price+(t.modifierTotal||0);r+=i*t.qty;let a=``;t.sugar&&t.sugar!==`Normal`&&(a+=`<p class="menu-desc" style="margin-bottom: 2px;">Gula: ${t.sugar}</p>`),t.selectedModifiers&&t.selectedModifiers.length>0&&t.selectedModifiers.forEach(e=>{let t=e.selected.map(e=>e.name).join(`, `);a+=`<p class="menu-desc" style="margin-bottom: 2px;">${e.groupName}: ${t}</p>`});let o=document.createElement(`div`);o.className=`cart-item`,o.innerHTML=`
